<template>
    <section class="select-none sm:pt-10 mx-4 my-4">
        ...
        <div class="bg-gray-900 rounded-lg mx-auto max-w-2xl px-4 py-6 sm:px-6 sm:py-6 lg:max-w-7xl lg:px-6 xl:hover:cursor-cell">
            <div ref="imageGrid" class="grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 xl:gap-x-8">
                <figure v-for="repo in visibleRepos" :key="repo.id" class="relative aspect-h-1 aspect-w-1 w-full overflow-hidden rounded-lg bg-gray-200 xl:aspect-h-8 xl:aspect-w-7 group">
                    <img :src="getImageSrc(repo.name)" :alt="repo.name" class="h-full w-full object-cover object-center" @error="handleImageError(repo.id)" />
                    <figcaption class="flex justify-around absolute w-full p-3 text-lg text-white bg-gradient-to-b from-transparent to-gray-700 bottom-0 opacity-0 transition-opacity duration-300 group-hover:opacity-100">
                        <div class="flex items-center">
                            <p class="mr-1">{{ repo.stargazers_count }}</p>
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#daaa3f" class="bi bi-star-fill" viewBox="0 0 16 16">
            ...
            <button v-if="showMoreButton" @click="toggleVisibility(true)" class="mt-4 px-4 py-2 bg-gradient-to-r from-violet-600 to-indigo-600 text-white rounded">
                Ver más
            </button>
            <button v-if="showLessButton" @click="toggleVisibility(false)" class="ml-2 mt-4 px-4 py-2 bg-gradient-to-r from-violet-600 to-indigo-600 text-white rounded">
                Ver menos
            </button>
        </div>
    </section>
...
interface Repo {
    id: string; // Cambia el tipo de id según sea necesario
    name: string;
...
const visibleCount = ref(numImagesLess); // Cantidad inicial de repositorios a mostrar
const allRepos = ref<Repo[]>([]); // Todos los repositorios obtenidos
...
// Computed para obtener los repositorios visibles
const visibleRepos = computed(() => {
    return allRepos.value.slice(0, visibleCount.value);
});
...
const handleImageError = (repoId: string) => {
    console.warn(`No se encontró imagen para el repositorio con id ${repoId}`);
    
    // Remover el repositorio sin imagen de allRepos
    allRepos.value = allRepos.value.filter(repo => repo.id !== repoId);
    updateButtons(); // Actualiza la visibilidad de los botones
};

// Función para alternar la visibilidad de los repositorios
const toggleVisibility = (showMore: boolean) => {
    if (showMore) {
        visibleCount.value += 6; // Aumentamos el número de repositorios visibles
    } else {
        visibleCount.value = Math.max(numImagesLess, visibleCount.value - 6); // Reducimos el número de repos visibles sin ir por debajo de numImagesLess
    }
    updateButtons(); // Actualiza la visibilidad de los botones
};
